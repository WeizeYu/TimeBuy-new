...
-(id)initWithViews:(NSArray*)imageArray
           withTag:(NSInteger)imagetag{
    self = [super init];
    if (self) {
        //check views length
        NSAssert(imageArray.count!=0, @".views's length is zero.");
        ...
        svrect_.size.height = self.view.bounds.size.height-90;
        ...
        svconsize.width = svrect_.size.width * imageArray.count;
...
//        [self.view addSubview:_button];
        ...
        int index_ = 0;
        for (UIImage *image_ in imageArray) {
    ...
    rect.origin.x = rect.size.width * (imagetag);
    [_scrollview scrollRectToVisible:rect animated:true];
    //添加手势，点击scrollview 退出
    //设置image的点击手势
    _scrollview.userInteractionEnabled = YES;
    UITapGestureRecognizer *singleTap1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(imageBigClick)];
    [_scrollview addGestureRecognizer:singleTap1];
